"""Run nnU-Net inference on test subjects, optionally to Morphologist format.

Usage:
    # Stage 1 - smoke test (2 subjects, prediction only)
    python scripts/infer_nnunet.py \\
        --work-dir /neurospin/.../hcp \\
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Convert to Morphologist .arg format (requires inference env).",
)
@click.option(
    "--checkpoint",
    default="best",
...
def main(
    ...
    if n_subjects is not None:
        ...
        print(
            f"Staged mode: using {len(selected)} of {len(all_images)} "
            "test subjects"
        )
    else:
        selected = all_images
    ...
    if result.returncode != 0:
        sys.exit(
            f"nnUNetv2_predict failed with exit code {result.returncode}"
        )
    ...
    if not morphologist:
        ...
        print(
            "  To convert, re-run with --morphologist in the inference env:\n"
            "    pixi run --environment inference python "
            f"scripts/infer_nnunet.py "
        ...
        try:
            ...
            sys.exit(
                ...
                "  pixi run --environment inference python "
                "scripts/infer_nnunet.py --work-dir ... --morphologist"
            )
        ...
        for seg_file in predictions:
            ...
            try:
                ...
                n_converted += 1
                print(
                    f"  [{n_converted}/{n_pred}] {subject_id}"
                    f"  -> {paths['arg'].name}"
                )
        ...
        print(
            f"\n  {n_converted}/{n_pred} subjects converted "
            f"in {elapsed2:.1f} s"
        )
