"""ABCD (Adolescent Brain Cognitive Development) dataset loader."""
...
class ABCDLoader(AbstractDataLoader):
    """
    Data loader for ABCD cohort (pediatric, multi-vendor, multi-site).

    Expected directory structure: same as UKBiobankLoader.
    Metadata CSV additionally expected to have: vendor, clinical_site_id columns.

    Stats:
        - 2,500 subjects (test set only)
        - Age: 9-10 years (pediatric baseline)
        - Scanners: Siemens, GE, Philips (3 vendors)
        - Sites: 21 sites across USA
