"""SENIOR cohort dataset loader (healthy aging, NeuroSpin)."""
...
class SENIORLoader(AbstractDataLoader):
    """
    Data loader for SENIOR cohort (healthy aging, NeuroSpin, 3T+7T).

    Expected directory structure: same as UKBiobankLoader.
    Metadata CSV additionally expected to have: field_strength column.

    Stats:
        - 142 subjects (all at baseline)
        - Age: 50-70 years (mean 60)
        - Scanners: 3T + 7T (ultra-high field)
        - Institution: NeuroSpin, France
        - Labels: Morphologist + FreeSurfer
        - Longitudinal: multiple timepoints available
    """

    VALID_SUBSETS = ("train", "val", "test")
    VALID_FIELD_STRENGTHS = ("3T", "7T")
